...
export async function withMongoTransaction<T>(work:(session:ClientSession)=>Promise<T>,_fallback?:()=>Promise<T>):Promise<T>{const session=await mongoose.startSession();try{let result!:T;await session.withTransaction(async()=>{result=await work(session)});return result}finally{await session.endSession()}}
...
const settled=['COMPLETED','APPROVED','PAID'];
...
async function rebuildWallet(userId:string,session?:ClientSession){
  ...
  const pending=await Withdrawal.aggregate([{$match:{userId:oid,status:{$in:['PENDING','PROCESSING']}}},{$group:{_id:null,total:{$sum:{$ifNull:['$reservedAmount','$amount']}}}}]).session(session||null);
...
async function ledgerInSession(data:LedgerInput,session:ClientSession){
  ...
  if(key){const existing=await Transaction.findOne({'metadata.idempotencyKey':String(key)}).session(session);if(existing)return existing}
...
export async function initializeWallets(){const users=await User.find().select('_id').lean();for(const user of users)await rebuildWallet(String(user._id));await Promise.all([Wallet.syncIndexes(),Transaction.syncIndexes(),Withdrawal.syncIndexes()])}
