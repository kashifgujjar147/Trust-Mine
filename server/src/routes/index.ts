...
import {paymentProofUpload,detectPaymentProofType} from '../middleware/paymentProofUpload.js';
...
route.post(
  ...
  async (req:any,res:any)=>{
    ...
    const detected = detectPaymentProofType(
      req.file.buffer
    );

    if(!detected || detected !== req.file.mimetype){
      return res.status(400).json({
        message:'Invalid payment screenshot file.'
      });
    }
    ...
    return res.status(201).json({
      ...
      proofUrl:
        `/api/deposits/payment-proof/${req.user.id}-${proofId}`
    });
  }
);

route.get(
  '/deposits/payment-proof/:ref',
  ...userAuth,
  async (req:any,res:any)=>{
    ...
    if(
      !file ||
      String(file.metadata?.userId) !== ownerId
    ){
      return res.status(404).end();
    }

    const stream = paymentProofStream(proofId);
...
route.post('/withdrawals',...userAuth,validate(withdrawalSchema),c.createWithdrawal);route.get('/withdrawals/eligibility',...userAuth,c.withdrawalEligibility);route.get('/withdrawals',...userAuth,c.withdrawals);route.get('/withdrawals/:id',...userAuth,c.withdrawalDetails);
