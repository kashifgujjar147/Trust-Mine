...
## User data
- GET `/users/dashboard` — JWT. Returns `balance`/`totalBalance`, `availableBalance`, `lockedWithdrawalAmount`, totals, active package purchases, recent transactions and settings. Available balance is computed server-side as total balance minus locked pending/processing withdrawals.
...
## Withdrawals
- POST `/withdrawals` — JWT. Body `{amount,method,account}`. Optional `Idempotency-Key` header. Backend validates minimum/payment method and atomically reserves the requested amount against the user's wallet; fee/net are calculated server-side. Pending/processing requests reduce available balance.
...
- POST `/admin/withdrawals/:id/approve` — PENDING → APPROVED; reservation remains locked.
- POST `/admin/withdrawals/:id/reject` — PENDING → REJECTED and releases reservation.
- POST `/admin/withdrawals/:id/processing` — APPROVED → PROCESSING.
- POST `/admin/withdrawals/:id/complete` — PROCESSING → COMPLETED and finalizes the reserved deduction.
...
### Financial rules
Default settings: minimum deposit `$2`, minimum withdrawal `$1`, withdrawal fee `8%`, commission rates `[10,2,1,1]`, reward tiers `[50→2,100→5,200→10,500→25]`, cycle interval `24h`, package duration `365 days`. Package `cycleDays` represents duration; the recurring business cycle uses the centralized `cycleIntervalHours` setting. Settings are stored in MongoDB and can be changed through admin settings. Frontend values are display-only; the server validates and calculates all financial effects.
