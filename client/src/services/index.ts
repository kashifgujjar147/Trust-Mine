...
export const authService={
  ...
  async requestPasswordReset(email:string){
    if(mockMode) return {message:'If the account exists, password-reset instructions will be sent.'};
    return (await api.post('/auth/forgot-password',{email})).data;
  },

  async resetPassword(token:string,password:string){
    if(mockMode) return {message:'Password updated successfully'};
...
export const adminService={
  ...
  async changeUserPassword(userId:string,password:string){
    if(mockMode){
      return delay({message:'User password updated successfully'});
    }
    ...
    if(mockMode){
      ...
      if(
        ...
        m.purchases.unshift({
          ...
          cycleEnd:new Date(
            Date.now()+
            Number(p.cycleDays||1)*
            Number(m.settings.cycleIntervalHours||24)*
            3600000
          ).toISOString(),
          currentCycle:0,
          accruedAmount:0,
