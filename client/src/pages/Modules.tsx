...
import {Copy,Check,ArrowRight,Send,LifeBuoy,CheckCircle,RefreshCw} from 'lucide-react';
...
export function Settings(){const [s,setS]=useState<PlatformSettings|null>(null),[error,setError]=useState('');const load=()=>settingsService.getSettings().then(setS).catch(e=>setError(e.message||'Unable to load settings.'));useEffect(()=>{void load()},[]);if(!s)return <Shell eyebrow="PREFERENCES" title="Settings" description="Centralized platform settings displayed from the settings service."><PageState type={error?'error':'loading'} message={error||'Loading settings.'} onRetry={error?load:undefined}/></Shell>;return <Shell eyebrow="PREFERENCES" title="Settings" description="Centralized platform settings displayed from the settings service."><div className="grid2"><Card><h3>Account security</h3><div className="settings-row"><span>Two-factor authentication</span><StatusBadge status="PENDING"/></div><div className="settings-row"><span>Session security</span><StatusBadge status="ACTIVE"/></div><Button variant="ghost" disabled>Change password unavailable</Button></Card><Card><h3>Platform defaults</h3><div className="settings-list"><div><span>Currency</span><b>{s.currency}</b></div><div><span>Minimum deposit</span><b>${s.minimumDeposit}</b></div><div><span>Minimum withdrawal</span><b>${s.minimumWithdrawal}</b></div><div><span>Withdrawal fee</span><b>{s.withdrawalFeePercent}%</b></div><div><span>Cycle interval</span><b>{s.cycleIntervalHours||24} hours</b></div><div><span>Package duration</span><b>{s.packageDurationDays||365} days</b></div></div></Card></div></Shell>}
