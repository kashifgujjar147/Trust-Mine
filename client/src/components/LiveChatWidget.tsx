...
export default function LiveChatWidget(){
  ...
  const unread=useMemo(
    ()=>tickets.reduce((sum,t)=>sum+(Number(t.unreadForUser)||0),0),
    [tickets]
  );
  ...
  const loadTickets=async()=>{
    try{
      ...
      if(activeTicket){
        const fresh=normalized.find((x:SupportTicket)=>x._id===activeTicket._id);
        if(fresh) setTicket(fresh);
      }
    }catch(e){
      setError((e as Error).message||'Unable to load support conversations.');
    }
  };
  ...
  useEffect(()=>{
    ...
    const first=async()=>{
      ...
      try{
        ...
        if(alive) setError((e as Error).message||'Unable to load support.');
      }finally{
  ...
  const createConversation=async(e:FormEvent)=>{
    ...
    try{
      ...
      setTickets(x=>[created,...x]);
      setNewConversation(false);
      setTicket(created);
      setSubject('');
      setMessage('');

      const data=await supportService.getMessages(created._id);
      setMessages(normalizeMessages(data));
    }catch(e){
  ...
  const send=async(e:FormEvent)=>{
    ...
    if(!activeTicket || !message.trim() || activeTicket.status==='RESOLVED') return;

    setSending(true);
    setError('');

    try{
      const sent=await supportService.sendMessage(
        activeTicket._id,
        message.trim()
      );
  ...
  return (
    <>
      ...
      >
        ...
        {unread>0 && <span className="live-chat-badge">{unread>99?'99+':unread}</span>}
      </button>
        ...
        <div className="live-chat-window">
          ...
          {!activeTicket ? (
            <div className="live-chat-body">
              ...
              {loading ? (
                <div className="live-chat-empty">Loading conversations...</div>
              ) : tickets.length>0 && !newConversation ? (
                <>
                  <div className="live-chat-section-title">Your conversations</div>
                    ...
                    {tickets.map(t=>(
                      ...
                      >
                        <div>
                          ...
                          <span>{t.lastMessagePreview||t.message}</span>
                        </div>
                ...
                <form onSubmit={createConversation} className="live-chat-form">
                  <div className="live-chat-section-title">Chat with support</div>
            ...
            <>
              ...
              <div className="live-chat-conversation-top">
                <button
                  type="button"
                  onClick={()=>{setTicket(null);setNewConversation(false);}}
              ...
              <div className="live-chat-messages">
                {messages.length===0 ? (
                  ...
                  messages.map((m,index)=>(
                    <div
                      key={`${m._id}-${index}`}
                      className={`live-chat-message ${m.senderRole==='USER'?'mine':'support'}`}
                    >
                      <div>{m.message}</div>
                      <small>
              ...
              {activeTicket.status==='RESOLVED' ? (
                ...
                <form onSubmit={send} className="live-chat-compose">
                  <textarea
                    className="field-input live-chat-textarea"
                  ...
                  <button className="btn" disabled={sending||!message.trim()}>
