...
export interface PackagePurchase{_id:string;packageId:string;packageName:string;packageAmount:number;status:Status;activatedAt:string;cycleStart:string;cycleEnd:string;currentCycle:number;accruedAmount:number;nextProcessAt:string;}
...
export interface Withdrawal{_id:string;transactionId:string;amount:number;fee:number;netAmount:number;method:string;account:string;status:Status;createdAt:string;reservedAmount?:number;idempotencyKey?:string;}
...
export interface SupportTicket{_id:string;subject:string;message:string;status:'OPEN'|'IN_PROGRESS'|'RESOLVED';createdAt:string;}
